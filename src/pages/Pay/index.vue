    ...
    <div class="paying container">
      <div class="header bgf">
        ...
        <div class="headerContent">
          <div class="part1">
            <p class="pay">应付</p>
            <p class="money">{{ payInfo.total }}积分</p>
          ...
          <div class="part2">
            ...
            <p class="timeout">
              ...
              <span class="decsTime" v-if="payInfo.orderId"
                >,剩余支付时间：{{ minutes }}分{{ seconds }}秒</span
              >
            </p>
            <p class="wenan">
  ...
  methods: {
    ...
    async getPayInfo() {
      ...
      if (result.data) {
        ...
        let count;
        //订单创建时间
        let { created } = this.payInfo;
        //格式化订单创建时间,并且转为毫秒
        let created_time = moment(
          created.slice(0, -5).replace("T", " ")
        ).format("x");
        //获取最新时间并转为毫秒
        let nowTime = moment(
          moment().utc().format("YYYY-MM-DD HH:mm:ss")
        ).format("x");
        //订单有效时间-(最新时间-订单创建时间)=剩余有效时间(单位为毫秒,所以/1000)
        this.decsTime = this.decsTime - (nowTime - created_time) / 1000;
        setInterval(() => {
          this.decsTime--;
          count = moment.duration(this.decsTime, "seconds");
          this.minutes = count._data.minutes;
          this.seconds = count._data.seconds;
        }, 1000);
      }
    },
    //发送验证码
